...
9. В реальном времени пишет строки лога в `reports/mission_*.log` рядом с `main.py`.

### Логи

При запуске `main.py` скрипт сразу создает файл вида `reports/mission_YYYYMMDD_HHMMSS.log` и во время выполнения миссии дописывает туда события: старт, маршрут, достижение ArUco, спавн препятствия, обратный маршрут и завершение.

Посмотреть логи после попытки:
